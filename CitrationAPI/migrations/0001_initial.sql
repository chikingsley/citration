...
CREATE INDEX sync_objects_workspace_version
  ON sync_objects (workspace_id, version);

CREATE INDEX sync_objects_type
  ON sync_objects (workspace_id, object_type, version);
...
CREATE TABLE sync_events (
  ...
  updated_by_device_id TEXT,
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (workspace_id, version, object_type, object_id),
  FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE,
  FOREIGN KEY (updated_by_device_id) REFERENCES devices(id) ON DELETE SET NULL
);
...
CREATE INDEX attachments_item
  ON attachments (workspace_id, item_id);
