...
The suite audit distinguishes small deterministic unit collaborators from product evidence. App-model tests use real temporary GRDB databases, credential files, attachment files, and the bundled CSL engine by default; controlled metadata, OCR, PDF-extraction, and HTTP collaborators test only their owning orchestration branch and are never cited as persistence, network, or compatibility proof. Test-only in-memory stores, fake DOI data, and the stub citation renderer were removed from production source, while the abandoned Citration account/session tests were deleted rather than treated as evidence for Zotero authentication.

The legacy SwiftData tests use a real temporary on-disk store, and the JSON-backed note, collection, relationship, attachment, annotation, and reader-progress tests use real temporary files. They characterize data that the completed one-time migration must continue to recognize, not the production architecture. Final persistence is covered by populated migration fixtures and `CitrationLibraryStore` tests using real SQLite databases and files. The connection profile is covered by a real SQLite database and a real permission-checked credential file, and live acceptance verified the production key through the same connection manager while proving the database has no secret column. A real temporary SQLite-and-files promotion test proves that connecting later copies local-only items, collections, notes, attachments, reader progress, and relationships into the remote library without deleting the local source, safely remaps key collisions, and performs no duplicate work on a second pass. On 2026-07-10 the production transport also completed a read-only live pull into a fresh database at version 1291: 414 items, 10 collections, 48 settings, 164 full-text records, 21 deletion keys, and zero groups or saved searches; an immediate incremental pull returned zero changes. A later self-cleaning CitrationCore drill passed a version-zero create, stale-version precondition, disjoint merge, same-field conflict, explicit resolution, synchronized deletion, integrity check, and independent cleanup verification against the live server. A separate live attachment drill passed the stock Zotero form transport, direct single R2 transport, 65 MiB multipart R2 transport with per-part retry, three MD5-verified downloads, independent remote cleanup, and database integrity. A live streaming drill then proved that subscription setup performs an initial authoritative pull and that remote create and delete notifications each trigger the ordinary incremental sync path before disposable cleanup. Final peer acceptance used a disposable Zotero Desktop profile and scoped device key: Desktop downloaded a Citration-created item, uploaded its edit, Citration received the edit through streaming-triggered normal pull, and Desktop observed Citration's deletion. The first complete run uncovered Zotero Self-Host's D1 bound-parameter failure on Desktop's 100-key reconciliation request; after the server was fixed, runtime-tested, deployed, and independently verified at 100 of 100 items, the repeated peer drill passed with database integrity `ok` and no remaining remote item, temporary key, Zotero process, or harness directory. A test double remains acceptable for a small pure unit boundary, but its result will continue to be labeled unit evidence only.

On 2026-07-11 the native iPad acceptance pulled a fresh integrity-clean version-1345 library with the same 414 items, 10 collections, 164 full-text records, 48 settings, zero groups, and zero saved searches. A separate self-cleaning live drill created a disposable parent, PDF attachment, and blue ink annotation, verified the exact parent key, page, sort index, color, paths, and width returned by the server, then independently verified all three disposable records were absent.
