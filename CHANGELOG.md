...
- Passed the final current-build acceptance matrix against Zotero Self-Host. A fresh read-only database pulled 414 items, 10 collections, 164 full-text records, 48 settings, and 41 deletion keys at library version 1340 with integrity `ok`. One disposable bidirectional item passed create, disjoint local/remote merge, preserved conflict, explicit resolution, synchronized deletion, and cleanup; an independent request returned `404`, both acceptance databases passed integrity checks, and no unresolved failure or live disposable row remained before the temporary directory was removed. The strict repository gate passed 108 core tests, the full Mac suite, the CLI build, API type/style checks, and all 6 contract tests. Visible production acceptance showed version 1340 Up to date, the real table and collections, complete selected-item fields, the preservation inventory of 636 raw objects with zero unsupported objects, and annotation/full-text search results before closing the app without mutation.
