...
struct SaaSEnvironmentTests {
    ...
    func buildsWorkspaceURLs() throws {
        let environment = try SaaSEnvironment(rootDomain: "citration.app")
        let slug = try WorkspaceSlug("acme-lab")

        #expect(environment.workspaceHost(for: slug) == "acme-lab.citration.app")
    ...
    func usesCustomAPIBaseURL() throws {
        ...
        let environment = try SaaSEnvironment(rootDomain: "citration.app", apiBaseURL: customAPI)
