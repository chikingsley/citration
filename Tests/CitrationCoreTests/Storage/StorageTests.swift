...
struct StorageTests {
    ...
    func localStoreBuildsDeterministicObjectKey() async throws {
        ...
        #expect(response.uploadURL.path.hasSuffix("/citration/AAAAAAAA-BBBB-CCCC-DDDD-EEEEEEEEEEEE/paper.pdf"))
    }
    ...
    func factoryReturnsLocalStoreForLocalConnector() {
        let factory = AttachmentObjectStoreFactory()
        let connector = StorageConnector(name: "local", type: .local, bucket: "ignored")
        let store = factory.makeStore(
            connector: connector,
            localBaseDirectory: URL(fileURLWithPath: "/tmp/citration")
        )
    ...
    @Test(
        "Factory returns S3CompatibleObjectStore for all remote types",
        arguments: [StorageConnectorType.s3, .r2, .supabaseS3, .minio]
    )
    func factoryReturnsS3CompatibleStoreForRemoteTypes(type: StorageConnectorType) {
        let factory = AttachmentObjectStoreFactory()
