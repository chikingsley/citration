...
- [x] Make single-tap selection remain in the library, then let an explicit Open action, Return, or double-click resolve and download the preferred readable attachment with visible inline progress and a compact chooser only when several documents are genuinely ambiguous.
...
- [x] Make the iPad PDF reader default to a complete fit-page reading experience and offer explicit display choices for paginated, continuous, and fit-width layouts.
- [x] Build one contextual iPad reader shell across PDF, EPUB, HTML, text, and DRM-free classic MOBI, adapting format-specific controls while keeping library return, Info, sharing, progress, search, and annotation surfaces coherent.
...
- [ ] Broaden the initial DRM-free classic MOBI reader beyond uncompressed and PalmDOC-compressed text: reconstruct embedded images and internal file-position links, add a real HUFF/CDIC fixture before claiming that compression mode, and continue rejecting encrypted publications explicitly.
