...
public extension CitrationLibraryStore {
    func listNotes(itemID: UUID?) throws -> [LibraryNote] {
        try database.databaseQueue.read { database in
            ...
            return try Row.fetchAll(
                database,
                sql: """
                SELECT note.note_html, identity.app_uuid, parent_identity.app_uuid AS parent_uuid,
    ...
    private static func decodeNote(_ row: Row) -> LibraryNote? {
